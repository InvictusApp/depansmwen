...
import com.klasscode.depansmwen.Model.database.DataBaseHandler;
...
public class AccountDao extends DaoBase implements DatabaseManager<Account> {
    ...
    public boolean insert(Account account) {
        ...
        long f = mDb.insert(TABLE_ACCOUNT,null, values);
        mDb.close();
    ...
    public boolean update(Account account) {
        ...
        long f = mDb.update(TABLE_ACCOUNT, values, KEY_ID + " = ?", new String[]{String.valueOf(account.getId())});
        return (f != 0) ? true : false;
    }
    ...
    public Account get(int id) {
        //SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = mDb.query(TABLE_ACCOUNT, new String[]{KEY_ID,ID_USER,BANK_NAME,NUMBER_ACCOUNT,BALANCE,
    ...
    public boolean delete(Account account) {
        //SQLiteDatabase db = this.getWritableDatabase();
        long f = mDb.delete(TABLE_ACCOUNT,KEY_ID + " = ?", new String[]{String.valueOf(account.getId())});
        mDb.close();
        return (f > 0) ? true : false;
    }
}
